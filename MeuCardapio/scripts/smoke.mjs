...
async function main() {
  ...
  try {
    ...
    await clickByText('[ ESC ] Cancelar', '.pos-sheet .btn')
    await waitFor(`document.body.innerText.includes('[ ENTER ] Aplicar forma de entrega')`, 'voltar para forma de entrega')
    ...
    await waitFor(`${toastHas('QR Mesa QR Smoke')} || ${toastHas('enviado para impressao')} || ${toastHas('fila')}`, 'qr impresso')

    const hasInventoryNav = await evaluate(`Boolean(document.querySelector('[data-testid="nav-inventory"]'))`)
    if (hasInventoryNav) {
